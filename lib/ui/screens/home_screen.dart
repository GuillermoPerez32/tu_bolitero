...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final showedResults = List<int>.empty(growable: true);
    ...
    return MultiBlocListener(
      ...
      child: BlocBuilder<LotteryCubit, LotteryState>(
                  ...
                  child: Column(
                    children: [
                      ...
                      state.maybeWhen(
                        ...
                        loaded: (_, followedPosts) => Expanded(
                          child: ListView.separated(
                            separatorBuilder: (context, index) =>
                                const SizedBox(height: 12),
                            itemCount: posts.length,
                            itemBuilder: (context, index) {
                              final postTile = PostTile(post: posts[index]);
                              if (showedResults
                                      .contains(posts[index].lottery.id) ||
                                  lotteryState.lotteries.isEmpty) {
                                return postTile;
                              }
                              showedResults.add(posts[index].lottery.id);
                              return Column(
                                children: [
                                  postTile,
                                  ResultCard(
                    ...
                    return Column(
                      ...
                      children: [
                        state.maybeWhen(
                          ...
                          loaded: (userData) {
                            return userData.esPronosticador ||
                                    userData.isSuperuser
                                ? FloatingActionButton(
                                    heroTag: 'add',
                                    onPressed: () => _openNumberInputSheet(
                                        context, postBloc),
